...
var K9sStylesFile = filepath.Join(USMgrHome(), "skin.yml")
...
type (
	...
	// Styles tracks USmgr styling options.
	Styles struct {
		USmgr       Style `yaml:"usmgr"`
		listeners []StyleListener
	}

	// Style tracks USmgr styles.
	Style struct {
		Body   Body   `yaml:"body"`
...
func (s *Styles) Update() {
	...
	tview.Styles.BorderColor = s.USmgr.Frame.Border.FgColor.Color()
